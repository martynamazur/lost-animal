...
class MapNotifier extends _$MapNotifier {
  ...
  Future<List<Marker>> _createMarkers(List<Report> reports) async {
    return await Future.wait(reports.map((report) async {
      final hasImage = report.pictures != null && report.pictures.isNotEmpty;


      BitmapDescriptor markerColor = BitmapDescriptor.defaultMarkerWithHue(
        report.type == 'seen' ? BitmapDescriptor.hueBlue : BitmapDescriptor.hueRose,
      );

      BitmapDescriptor markerIcon;
      if (hasImage) {
        try {
          markerIcon = await createCustomMarkerWithRoundedImage( report.type, report.pictures.first,);
        } catch (_) {
